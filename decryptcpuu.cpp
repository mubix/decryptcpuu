...
int main(int argc, char *argv[])
{
    ...
    // 0C01 (2 bytes)
    byte iniPubkey[268];
    // TODO: Check to see if the size is at least X number of bytes with a divisible 16 at the end
    size_t iniEncPassLen = (inputLen/2) - 2 - 268;
    byte * iniEncpasswd = (byte *) malloc(iniEncPassLen);


    for (size_t input_idx = 2, pubkey_idx = 0; pubkey_idx < sizeof(iniPubkey); input_idx++, pubkey_idx++)
    {
        char v;
    ...
    print_buf("INI Pubkey", iniPubkey, sizeof(iniPubkey));


    for (size_t input_idx = 270, pubkey_idx = 0; pubkey_idx < iniEncPassLen; input_idx++, pubkey_idx++)
    {
        char v;
        if (sscanf_s(argv[1] + (input_idx * 2), "%2hhx", &v) != 1)
        {
    ...
    print_buf("\nEncrypted Password:", iniEncpasswd, iniEncPassLen);
    HCRYPTKEY phDoubleKey;
    final = CryptImportKey(phProv, iniPubkey, sizeof(iniPubkey), phKey, 0, &phDoubleKey);
    printf("GetLastError Code: %d\n", GetLastError());
    printf("Did CryptImportKey on the Pub Key work? %d\n", final);

    final = CryptDecrypt(phDoubleKey, NULL, TRUE, 0, iniEncpasswd, (DWORD *)&iniEncPassLen);
    printf("%ls\n", iniEncpasswd);
